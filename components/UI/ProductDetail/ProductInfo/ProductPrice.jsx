			...
			<div className={styles.wrapper}>
				{(see_price === 'login' && loginUser?.id !== undefined) || see_price === 'without_login' ? (
					<>
						<div className={styles.inner_wrapper}>
							{changingData?.type === 'variation' ? (
								<div className={styles.multiple_sku}>
									...
									<div className={styles.price}>{data?.product?.range}</div>
								</div>
							) : (
										...
										<span className={styles.price}>
											{changingData?.price !== 0 && changingData?.price !== null
												? changingData?.price_discounted !== null && changingData?.price_discounted !== 0 && changingData?.price_discounted !== changingData?.price_catalog && (changingData?.price_discounted_end === null || new Date(changingData?.price_discounted_end) >= new Date())
													? currencyFormat(changingData?.price_discounted)
													: currencyFormat(changingData?.price)
												: 'Price N/A'}
										</span>
									</div>
								</div>
						...
						{changingData?.price_discounted !== null && changingData?.price_discounted !== 0 && (changingData?.price_discounted_end === null || new Date(changingData?.price_discounted_end) >= new Date())
							? changingData?.type !== 'variation' &&
					...
					see_price === 'login' && (
						<Link href='/auth/signin'>
							<a>
								<button className={`${styles.hide_price} sml-btn red-color`}>Signin to see the price</button>
							</a>
						</Link>
					)
				)}
				<div className={styles.inner_wrapper}>
					...
					<div className='views-num'>{data?.product_views} Viewed</div>
				</div>
			</div>
			{((see_price === 'login' && loginUser?.id !== undefined) || see_price === 'without_login') && (
				<>
