...
const CardModal = ({ modal, setmodal, className = '', registerType, children, modalType, pageType, setformsubmit, displayType, mobSearchSidebar, alertType, setcheckingUserData, setmodalAlertType, modalAlertType }) => {
	...
	return (
		<div ref={modalRef} className={`${styles.modal_container} modal-container modal-bg transition ${modal ? 'show-bd' : 'hide-bd'} ${displayType === 'sidebar' && styles.sidebar} ${modalType === 'video' && styles.stack_modal}`}>
			<div
				ref={wrapperRef}
				className={`${styles.inner_wrapper} ${modalType === 'crop' ? styles.crop_modal : undefined} ${registerType === 'ListingPage' ? className : undefined} ${displayType === 'sidebar' ? styles.sidebar_wrapper : undefined} ${
					modal && (displayType === 'sidebar' ? styles.show_sidebar : styles.show_modal_wrapper)
				}  ${modalType !== 'video' && 'white-bg'} ${displayType !== 'sidebar' ? 'radius' : undefined} transition ${modalType == 'review' ? styles.review_modal : undefined} ${modalType === 'expo-register' || modalType === 'expo-form' ? styles.webinar_form : undefined} modal-wrapper`}
			...
			>
				...
				{modalType !== 'video' && modalType !== 'product' && modalType !== 'expo-register' && modalType !== 'reviews' && modalType !== 'meeting' && <div className={`${styles.gradient} radius`} />}
				<div className={`${styles.modal_wrapper} ${modalType === 'crop' ? styles.crop_wrapper : undefined} ${registerType !== 'ListingPage' ? className : undefined}  ${pageType === 'address-listing' && styles.form_wrapper} transition `}>{children}</div>
			</div>
		</div>
	)
}
