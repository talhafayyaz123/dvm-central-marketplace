...
const ImgsGallery = ({ data, videos, images, name, setsidebarVideoModalData, setsidebarVideoModal }) => {
	const { price, price_catalog, type, price_discounted, price_discounted_end, see_price } = data

	console.log('see_price', see_price)
	...
	const openVideoModalHandler = (video) => {
		setsidebarVideoModalData(video)
		setsidebarVideoModal(true)
	}
	...
	return (
		<>
			...
			{/* big imgs gallery */}
			<div className={`${styles.img_wrapper} img_wrapper2 shadow ${styles.sidebar_imgs}`}>
				{see_price !== 'request' &&
				...
				{images?.length > 0 ? (
								...
								return (
									<SwiperSlide key={vid.id}>
										<div className={styles.vid_wrapper} onClick={() => openVideoModalHandler(vid)}>
											...
											<ImgWithLoader className={`${styles.video_img}`} width={350} height={350} src={vid?.video_image !== null ? `${imgApiUrl.products.videos.images}/${vid?.video_image}` : placeHolderImg} alt={name} />
										</div>
									</SwiperSlide>
								)
							})}
					...
					images?.length === 0 && <ImgWithLoader width={350} height={350} src={placeHolderImg} alt={name} />
				)}
			</div>
							...
							images?.map((img) => {
								return (
									<SwiperSlide key={img.id}>
										<ImgWithLoader className={`${styles.thumb_img} transition`} width={80} height={80} src={img.name !== null ? `${imgApiUrl.products.thumbnail}/${img.name}` : placeHolderImg} alt={name} />
									</SwiperSlide>
								)
							})}
						{videos?.length > 0 &&
