...
const VendorDetail = ({ sku, data, name, closeSidebar, productType, vendorSlug, quickFacts }) => {
		...
		<div className={styles.vendor_wrapper}>
			<div className={styles.inner_wrapper}>
				...
				<Link href={`/vendors/${vendorSlug}`}>
					<a className='primary-color link semibold-text' onClick={() => closeSidebar()}>
						{' '}
						{name}
					</a>
				</Link>
			...
			{productType !== 'variation' && (
				<div className={styles.inner_wrapper}>
					<div>SKU : </div>
					<div className='semibold-text'> {sku}</div>
				</div>
			)}
			...
			{brands?.length > 0 && (
				<div className={styles.inner_wrapper}>
					...
					<div className={styles.brands_wrapper}>
						{brands?.map((brand, i) => (
							<div key={i} className='semibold-text'>
						...
						{data?.map((productCategory, index) => {
							...
							return (
								<Link href={`/${slug}`} key={index}>
									<a onClick={() => closeSidebar()} className={`${styles.product_category} primary-color semibold-text`}>
