...
const ProductRating = ({ data, userPosition }) => {
	const { service_rating, unit_sold, reviews_count } = data
	...
	const addToWishlist = async (id) => {
		...
		console.log('res from wishlist service', res)
	...
	useEffect(() => {
		let findWishlistProduct = wishlistData?.serviceWishlist?.find((service) => {
			return service?.service_id === data?.service?.id
		})
	...
	return (
		<div className={styles.wrapper}>
			{service_rating !== null && service_rating !== 0 && service_rating !== undefined && (
				<>
					<div className={styles.inner_wrapper}>
						<div className={styles.star_rating}>
							<div className={`${styles.count} orange-color`}>{service_rating}/5</div> <Rating data={service_rating} /> <div className='gray-color'>{`(${reviews_count})`}</div>
						</div>
					</div>
			...
			{loginUser?.id !== undefined && userPosition !== undefined && userPosition !== 'Sales Rep' && (
				<button className={styles.wish_wrapper} onClick={() => addToWishlist(data?.service?.id)}>
