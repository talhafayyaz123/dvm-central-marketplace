...
const ShopByDepartments = ({ showShopByDepts, setShowByDepts }) => {
	...
	return (
		<div className={`${styles.shop_by_dpts_container} shop-by-dpts-container modal-bg transition ${showShopByDepts ? 'show-bd' : 'hide-bd'}`}>
			<div className={`${styles.shop_by_depts_wrapper} shop-by-dpts-wrapper sidebar white-bg transition ${showShopByDepts ? 'show-sidebar' : undefined} ${hideScroller ? 'hidden-scroller' : undefined}`} ref={shopByContainer}>
				...
				<div className='shop-by-depts-lists-wrapper'>
					<ShopByDeptsLists userData={userData} showShopByDepts={showShopByDepts} setShowByDepts={setShowByDepts} shopByContainer={shopByContainer} setHideScroller={setHideScroller} loginUser={loginUser} setloginUser={setloginUser} />
				</div>
			</div>
		</div>
	)
