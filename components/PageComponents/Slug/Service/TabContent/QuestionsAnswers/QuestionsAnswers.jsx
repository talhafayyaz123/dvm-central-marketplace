...
const QuestionsAnswers = ({ vendorId, qaData, showQA, setqaData, qaLoading, getQAData, serviceId, loginUser }) => {
	...
	const submitAnswerhandler = async (id) => {
		...
		if (!inputValidation) {
			...
			await axios.post(`${baseApiUrl}/product-answer`, data)
							...
							<div className={styles.qa_listing_wrapper}>
								...
								{qaData?.length > 0
									? qaData?.map((qa, i) => {
											return (
												qa?.question !== null && (
													<div key={i} className={`${styles.qa_wrapper}`}>
