...
const QAS = ({ qaData, showQA, setqaData, qaLoading, getQAData, vendorId, serviceId }) => {
	...
	const questionSearchHandler = async (val) => {
		...
		const res = await axios.post(`${baseApiUrl}/product-service-question`, data)
		...
		showQA && (
			<div>
				...
				<div className={styles.qa_listing_wrapper}>
					{qaLoading && <DarkLoader className={styles.qa_loader} />}
					{qaData?.length > 0
						? qaData?.map((qa) => {
								return (
									qa?.question !== null && (
										<div key={qa?.id} className={`${styles.qa_wrapper}`}>
											<div className={styles.q_wrapper}>
												...
												<p className='primary-color'>{qa?.question}</p>
											</div>
	...
	// : (
	// 	!qaLoading && <div className='red-color'>No questions and answers found for this product.</div>
	// )
}
