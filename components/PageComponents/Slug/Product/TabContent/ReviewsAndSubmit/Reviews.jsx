...
const ReviewsAndSubmit = ({ productId, rating, ratingBars, reviewCount }) => {
	...
	const getReviews = async () => {
		const res = await axios.get(`${baseApiUrl}/get-reviews/${productId}`)
	...
	const fetchData = async () => {
		const res = await axios.get(`${baseApiUrl}/get-reviews/${productId}?page=${currentPage + 1}`)
				...
				{modal && (
					<div className={styles.modal_wrapper}>
						...
						<div className={styles.imgs_gallery}>
								...
								{modalVideoSrc !== null && (
									<SwiperSlide>
										<div className={`${styles.play_icon} gray-border ${styles.img}`}>
											<svg xmlns='http://www.w3.org/2000/svg' fill='var(--primary)' viewBox='0 0 24 24' strokeWidth={0.75} stroke='var(--white)'>
							...
							<svg xmlns='http://www.w3.org/2000/svg' className={`${styles.next_btn} review-button-next white-bg primary-border full-radius`} fill='none' viewBox='0 0 24 24' stroke='var(--primary)'>
								<path strokeLinecap='round' strokeLinejoin='round' strokeWidth={1.5} d='M9 5l7 7-7 7'></path>
							</svg>

							<svg xmlns='http://www.w3.org/2000/svg' className={`${styles.prev_btn} review-button-prev white-bg primary-border full-radius`} fill='none' viewBox='0 0 24 24' stroke='var(--primary)'>
				...
				<div className={styles.wrapper}>
					{reviewsData?.length > 0 ? (
						reviewsData?.map((data, index) => {
							const { name, purchased, video, rating, vendor_approved, admin_approved, time, review_images, comments, customer } = data

							const [date, localTime12HoursBase, pmAm] = getTimezoneDate(time)
							return (
								comments !== null &&
								comments !== '' && (
									<div key={index} className={`${styles.comment} primary-bb shadow radius`}>
	...
	c-6.617,0-12,5.383-12,12C31,31.553,30.553,32,30,32z'
										...
										{purchased === 'Y' && vendor_approved === 'Y' && admin_approved === 'Y' ? <div className={`${styles.batch} ${styles.verified}`}>Verified Purchase </div> : <div className={`${styles.batch} ${styles.not_verified}`}>Not Verified</div>}

										<div className={`gray-color ${styles.msg}`}>{comments}</div>
										<div className={`gray-color ${styles.time}`}>
											{date} {localTime12HoursBase?.toString()?.slice(0, localTime12HoursBase?.toString()?.lastIndexOf(':'))} {pmAm}
										</div>
									</div>
								)
							)
