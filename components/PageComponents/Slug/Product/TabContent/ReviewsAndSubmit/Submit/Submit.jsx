...
const Submit = ({ productId, modal, setmodal, loginUser, userPosition, className }) => {
	...
	const compressedImg = async (image) => {
		if (image?.length > 0) {
			...
			const blob1 = await getBlobImg(compressedimage)
			return await fetch(blob1).then((r) => r.blob())
		} else return
	}
	...
	const onSubmit = async (data) => {
		...
		!loginUser && formData.append('name', data?.name)
		formData.append('email', loginUser ? loginUser?.email : data?.email)
		formData.append('comments', data?.review)
		formData.append('rating', data?.rating)
		formData.append('images[1]', blobImg1, data?.review_img1[0]?.name)
		data?.review_img2?.length > 0 && formData.append('images[2]', blobImg2, data?.review_img2[0]?.name)
		data?.review_img3?.length > 0 && formData.append('images[3]', blobImg3, data?.review_img3[0]?.name)
		data?.review_img4?.length > 0 && formData.append('images[4]', blobImg4, data?.review_img4[0]?.name)
		data?.review_img5?.length > 0 && formData.append('images[5]', blobImg5, data?.review_img5[0]?.name)
		...
		// coins
		if (res?.data?.success && loginUser?.id !== undefined && userPosition !== 'Sales Rep') {
			const userData = {
